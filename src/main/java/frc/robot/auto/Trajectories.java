...
public class Trajectories {
    ...
    public Trajectory getLeftStartToSafeForward(){
        ...
        // Using Json files for paths

        public Trajectory getTrenchfromJson(){
            Trajectory trenchJson = null;
            try {
                Path trajectoryPath = Filesystem.getDeployDirectory().toPath()
                    .resolve("paths/trenchRun.wpilib.json");
                trenchJson = TrajectoryUtil.fromPathweaverJson(trajectoryPath);
              } catch (IOException ex) {
                DriverStation.reportError("Unable to open trajectory: " + "paths/trenchRun.wpilib.json",
                    ex.getStackTrace());
              }
            return trenchJson;
        }
